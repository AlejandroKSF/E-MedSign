    ...
    <main class="cabeça">
        <div class="tablesInd">
            <div class="titulo-paciente">
                <h3 class="index">Medicos cadastrados</h3>
            </div>
            ...
            <table class="tabela-paciente">
                <thead class="tabela-titulo">
                    <tr class="cabeça-tabela-paciente">
                        ...
                        <th class="titulo-cabecalho">Especialização</th>
                        <th class="titulo-cabecalho">CRM</th>
                        <th class="titulo-cabecalho">Clinica</th>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr class='campo-tabela' id='row_" . htmlspecialchars($row['cd_funcionario']) . "'>";
                            echo "<td class='informacaion-tabela'>" . htmlspecialchars($row['nm_funcionario']) . "</td>";
                            echo "<td class='informacaion-tabela'>" . htmlspecialchars($row['ds_email']) . "</td>";
                            echo "<td class='informacaion-tabela'>" . htmlspecialchars($row['ds_Especializacao']) . "</td>";
                            echo "<td class='informacaion-tabela'>" . htmlspecialchars($row['cd_Crm']) . "</td>";
                            echo "<td class='informacaion-tabela'>" . htmlspecialchars($row['nm_clinica']) . "</td>";

                            echo "<td class='informacaion-tabela'>
                                <a class='link-icon' href='editar_Medico.php?id=" . htmlspecialchars($row['cd_funcionario']) . "' aria-label='Edit'><i class='bi bi-pencil'></i></a>  
                                <a class='link-icon delete-paciente' href='#' data-id='" . htmlspecialchars($row['cd_funcionario']) . "' aria-label='Delete'><i class='bi bi-trash3'></i></a>
                            </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='4'>Nenhum médico cadastrado.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
